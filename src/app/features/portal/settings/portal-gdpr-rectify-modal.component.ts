...
export class PortalGdprRectifyModalComponent {
  ...
  async submit(): Promise<void> {
    ...
    try {
      // Call the portal-create-arc-request Edge Function (deployed at
      // supabase/functions/portal-create-arc-request). Bypasses PostgREST
      // entirely so we sidestep the project's stuck schema cache. The
      // function uses service_role to write to gdpr_access_requests +
      // gdpr_audit_log and the existing on_gdpr_request_created trigger
      // fires the notification to the owner.
      const { data, error } = await this.auth.client.functions.invoke<
        | { success: true; request_id: string }
        | { success: false; error: string }
      >('portal-create-arc-request', {
        body: {
